...
def test_maca_qk_and_pv_mma_tiles(tmp_path):
    ...
    source = Path(__file__).with_name("maca_attention_tile_probe.cpp")
    binary = tmp_path / "maca_attention_tile_probe"
    compile_cmd = [
        str(mxcc),
        "-x",
        "maca",
    ...
    assert "QK tile PASS" in result.stdout
    assert "PV tile PASS" in result.stdout
    assert "MACA attention tile probe PASS" in result.stdout
