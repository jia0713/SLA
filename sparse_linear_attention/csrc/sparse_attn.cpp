...
torch::Tensor sparse_attn_forward(
  ...
  TORCH_CHECK(block_m == 64 || block_m == 128, "block_m must be 64 or 128");
  TORCH_CHECK(block_n == 64, "block_n must be 64");
...
torch::Tensor sparse_attn_forward_cute(
    torch::Tensor q,
    torch::Tensor k,
    torch::Tensor v,
    torch::Tensor lut,
  ...
  TORCH_CHECK(block_n == 64, "block_n must be 64");
  TORCH_CHECK(q.size(3) == 64 || q.size(3) == 128, "head_dim must be 64 or 128");
  return sparse_attn_forward_cute_cuda(q, k, v, lut, topk, block_m, block_n);
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("forward", &sparse_attn_forward, "SLA sparse attention forward (CUDA)");
  m.def("forward_cute", &sparse_attn_forward_cute, "SLA sparse attention forward (MACA CUTE)");
}
